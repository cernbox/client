...
Source0:	cernbox-%{version}.tar.bz2
...
Requires:	libcernboxsync = %{version}
...
%package doc
Summary:        Documentation for CERNBox Client
Group:          Development/Libraries/C and C++
Requires: %{name}%{?_isa} = %{version}-%{release}

%description doc
Documentation about the CERNBox Client desktop application.

%package l10n
Summary:        Localisation for CERNBox Client
Group:          Development/Libraries/C and C++
Requires: 	%{name}%{?_isa} = %{version}-%{release}

%description l10n
Localisation files for the CERNBox Client desktop application.

%package -n libcernboxsync
Requires:       %{?sclnameprefix}qtkeychain >= %{qtkeychain_min_ver}
%{?el7:
Requires:	neon >= %{neon_min_ver}
}
...
Summary:        The CERNBox sync library
Group:          Development/Libraries/C and C++

%description -n libcernboxsync
The CERNBox client sync library.

%package -n libcernboxsync-devel
Summary:        Development files for CERNBox sync library
Group:          Development/Libraries/C and C++
Requires: 	libcernboxsync = %{version}

%description -n libcernboxsync-devel
Development files for the CERNBox client sync library.
...
env LD_RUN_PATH=%{_libdir}/cernbox make %{?_smp_mflags} VERBOSE=1
...
%if "%{name}" != "owncloud-client"
  mv ${RPM_BUILD_ROOT}%{_mandir}/man1/{owncloud.1,cernbox.1}
  mv ${RPM_BUILD_ROOT}%{_mandir}/man1/{owncloudcmd.1,cernboxcmd.1}
%endif
  gzip ${RPM_BUILD_ROOT}%{_mandir}/man1/*.1
fi

%define extdir ${RPM_BUILD_ROOT}%{_datadir}/nautilus-python/extensions
test -f %{extdir}/ownCloud.py  && mv %{extdir}/ownCloud.py  %{extdir}/cernbox.py  || true
test -f %{extdir}/ownCloud.pyo && mv %{extdir}/ownCloud.pyo %{extdir}/cernbox.pyo || true
test -f %{extdir}/ownCloud.pyc && mv %{extdir}/ownCloud.pyc %{extdir}/cernbox.pyc || true


# make the wrapper for commands
%{?usescl:
mkdir -p ${RPM_BUILD_ROOT}%{_libexecdir}/cernbox

mv ${RPM_BUILD_ROOT}%{_bindir}/cernbox ${RPM_BUILD_ROOT}%{_libexecdir}/cernbox/
mv ${RPM_BUILD_ROOT}%{_bindir}/cernboxcmd ${RPM_BUILD_ROOT}%{_libexecdir}/cernbox/

cat << EOF > ${RPM_BUILD_ROOT}%{_bindir}/cernbox
#!/bin/bash
source %{usesclbase}/enable
%{_libexecdir}/cernbox/cernbox \$*
EOF

chmod 0755 ${RPM_BUILD_ROOT}%{_bindir}/cernbox

cat << EOF > ${RPM_BUILD_ROOT}%{_bindir}/cernboxcmd
#!/bin/bash
source %{usesclbase}/enable
%{_libexecdir}/cernbox/cernboxcmd \$*
EOF

chmod 0755 ${RPM_BUILD_ROOT}%{_bindir}/cernboxcmd
}

# original file is really for SuSE , not Red Hat .. let's make a new one.
rm -f ${RPM_BUILD_ROOT}/%{_datadir}/applications/cernbox.desktop

cat << EOF > ${RPM_BUILD_ROOT}/%{_datadir}/applications/cernbox.desktop
[Desktop Entry]
Version=1.0
Type=Application
...
%{_sysconfdir}/cernbox/sync-exclude.lst
# this file should be validate with desktop-file-install
%{_datadir}/applications/cernbox.desktop
%{_datadir}/icons/hicolor
%if 0%{have_doc}
%{_mandir}/man1/cernbox*
%endif
...
%files -n %{?scl_prefix}libcernboxsync
%defattr(-,root,root,-)
%{_libdir}/libcernboxsync.so.*
%dir %{_libdir}/cernbox
%{_libdir}/cernbox/libocsync.so.*

%files -n %{?scl_prefix}libcernboxsync-devel
%defattr(-,root,root,-)
%{_libdir}/libcernboxsync.so
%{_libdir}/libhttpbf.a
%{_libdir}/cernbox/libocsync.so
%{_includedir}/cernboxsync/
